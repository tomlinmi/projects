...
function printCartInventory(cart) {
 for(let cartItem in cart){
   const itemName = cart[cartItem];//["Gold Round Sunglasses", "Pink Bucket Hat"]
   const quantity = itemName.quantity;//["1", "2"] 
   qtyJoinedItem =quantity.join("x");//"1x"
   itemJoinedItem=cartItem.join("\n");//"Gold Round Sunglasses\n"
 }
 
return `${qtyJoinedItem}` + `${itemJoinedItem}`;
}
