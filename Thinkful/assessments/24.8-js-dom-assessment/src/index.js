...
function render(contacts) {
  ...
  const main = document.querySelector("#contacts"); main.innerHTML = "";
...
function filterByCity(city) {
  const result = contacts.filter((contact)=>contact.address.city===city)
...
function loadCities(contacts) {
  ...
  for(let i=0; i<contacts.length; i++){
    results.push(contacts[i].address.city)  
   }
   
   const filterOptions = document.querySelector("#filterOptions") 
   if(results.length !=0){
       let uniquecity = [...new Set(results)];
     for(let i=0; i<uniquecity.length; i++){
       const option = document.createElement("option")
       //set key and value to city
       option.value = uniquecity[i];
       option.text = uniquecity[i];   
...
deleteBtns.forEach( (deleteBtn) => { deleteBtn.addEventListener ( "click", (event) => { const id = event.target.parentNode.getAttribute("data-id"); deleteContact(id); render(contacts); 
  });                                  
    });
